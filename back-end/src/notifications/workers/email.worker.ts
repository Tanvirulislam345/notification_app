...
import { EMAIL_QUEUE, NotificationJob } from '../../queue/queue.constants';
...
@Processor(EMAIL_QUEUE, {
  concurrency: parseInt(process.env.EMAIL_WORKER_CONCURRENCY ?? '25', 10),
  settings: { backoffStrategy: (attemptsMade: number) => notifBackoff(attemptsMade) },
})
export class EmailWorker extends BaseChannelProcessor {
  constructor(
    ...
    super(Channel.EMAIL, resolver, logs, metrics, logger, dlq);
  }
